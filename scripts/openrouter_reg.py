...
# ─── DuckMail / Outlook 轮询（提取 Clerk 验证链接） ──────────────────────────
def _poll_duckmail(
    email: str, meta: dict,
    timeout: int = 180,
    log_q: Optional[queue.Queue] = None,
    ...
    while time.time() - start < timeout:
        ...
        attempt += 1
        if log_q is not None and attempt % 5 == 1 and attempt > 1:
            log_q.put(f"等待验证邮件 (duckmail)... ({int(time.time()-start)}s / {timeout}s)")
        try:
            ...
            consecutive_errors = 0
            for msg in members:
                link = _extract_verify_link(msg.get("subject", "") or "")
                if link:
                    return link
                msg_id = msg.get("id", "")
                if not msg_id:
...
def _poll_outlook_or(
    email: str, meta: dict,
    timeout: int = 180,
    log_q: Optional[queue.Queue] = None,
    cancel: Optional[Event] = None,
) -> Optional[str]:
    """Outlook 账号池：IMAP XOAUTH2 收件，提取 Clerk 验证链接（复用 outlook_mail）。"""
    ...
    if log_q is not None:
        log_q.put("轮询 Outlook 收件箱 (IMAP)...")
    return poll_outlook_extract(email, cid, rt, _extract_verify_link, timeout=timeout, senders=None, cancel=cancel)
