...
// apiKey: API Key（如 AC-xxxx）；支持多行、逗号或分号分隔多个 key。
func NewYYDSMailProvider(baseURL, apiKey string) *YYDSMailProvider {
	return &YYDSMailProvider{
		...
		apiKeys:    parseYYDSMailAPIKeys(apiKey),
...
func parseYYDSMailAPIKeys(raw string) []string {
...
func (p *YYDSMailProvider) apiKeyAttempts(baseRetries int) []string {
	if baseRetries < 1 {
		baseRetries = 1
	}
	if len(p.apiKeys) == 0 {
		return makeRepeatedYYDSMailAPIKeys("", baseRetries)
	}
	if len(p.apiKeys) == 1 {
		return makeRepeatedYYDSMailAPIKeys(p.apiKeys[0], baseRetries)
	}

	attempts := baseRetries
	if len(p.apiKeys) > attempts {
		attempts = len(p.apiKeys)
	}

	out := make([]string, 0, attempts)
	for len(out) < attempts {
		for _, key := range shuffleYYDSMailAPIKeys(p.apiKeys) {
			out = append(out, key)
			if len(out) == attempts {
				break
			}
...
func makeRepeatedYYDSMailAPIKeys(key string, count int) []string {
...
func shuffleYYDSMailAPIKeys(keys []string) []string {
	shuffled := append([]string(nil), keys...)
	for i := len(shuffled) - 1; i > 0; i-- {
		j := 0
		if n, err := rand.Int(rand.Reader, big.NewInt(int64(i+1))); err == nil {
			j = int(n.Int64())
		} else {
...
func (p *YYDSMailProvider) doRequest(ctx context.Context, method, rawURL string, authToken string, maxRetries int) ([]byte, error) {
	var lastErr error
	apiKeyAttempts := []string{}
	if authToken == "" {
		apiKeyAttempts = p.apiKeyAttempts(maxRetries)
		maxRetries = len(apiKeyAttempts)
	}
	for i := 0; i < maxRetries; i++ {
		...
		} else if apiKeyAttempts[i] != "" {
			req.Header.Set("X-API-Key", apiKeyAttempts[i])
		}
		...
		if resp.StatusCode == 401 || resp.StatusCode == 403 {
			if authToken == "" && len(p.apiKeys) > 1 && i < maxRetries-1 {
				lastErr = fmt.Errorf("%w: yydsmail HTTP %d", ErrAuthFailed, resp.StatusCode)
				continue
			}
		...
		if resp.StatusCode == 429 {
			if authToken == "" && len(p.apiKeys) > 1 && i < maxRetries-1 {
				lastErr = fmt.Errorf("%w: yydsmail 速率限制", ErrRateLimited)
				continue
			}
...
func (p *YYDSMailProvider) GenerateEmail(ctx context.Context) (string, map[string]string, error) {
	...
	if resp.Data.Token == "" {
		return "", nil, fmt.Errorf("yydsmail 返回缺少 token")
	}
	if resp.Data.ID == "" {
		return "", nil, fmt.Errorf("yydsmail 返回缺少 inbox id")
	}
